...
<template>
  <div class="relative h-screen w-full">
    <nav
      class="h-15 w-full flex items-center justify-between border-b px-10"
    >
      <NuxtLink
      ...
      >
        <svg
          ...
          aria-hidden="true"
          class="iconify iconify--noto h-full w-full"
        >
          <path
      ...
      <div class="h-10 w-10 overflow-hidden rounded-50%">
        <img :src="userStore.picture">
      </div>
    </nav>

    <main class="h-[calc(100vh-60px)] w-full flex items-center justify-center p-10">
      <div class="h-full max-w-360 w-full border">
        <slot />
      </div>
    </main>
  </div>
